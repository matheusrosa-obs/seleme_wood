...
if st.session_state.pagina == "Demanda 1":
    ...
    # moved Empresas por Canal section to its own page (see below)
...
################### PÁGINA 1.5 ###################
    ...
    # Coordenadas fictícias para mostrar no mapa (placeholder)
    placeholder_df = placeholder_df.assign(nu_latitude=[-27.6, -26.9, -29.2], nu_longitude=[-48.6, -49.3, -51.2])

    with tab_ecom:
        st.markdown("Breve explicação: nesta aba colocaremos os e-commerces que atuam como consumidores ou revendedores. (Placeholder)")
        c1, c2 = st.columns([1, 1])
        with c1:
            st.markdown("**Tabela (placeholder)**")
            st.dataframe(placeholder_df[["Município", "UF", "Quantidade"]], height=300)
        with c2:
            ...
            fig_e.update_layout(mapbox_style="carto-darkmatter", margin=dict(l=0, r=0, t=0, b=0), height=300)
            st.plotly_chart(fig_e, use_container_width=True)

    with tab_port:
        st.markdown("Breve explicação: empresas do portfólio — foco em quem trabalha com painéis de pinus.")
        # Carregar dados (serão cacheados pela função load_data)
        try:
            df_companies = load_data("App/companies_partial.csv")
            df_items = load_data("App/items_partial.csv")
        ...
        works_flag = df_companies.get("works_with_pine_panel")
        pine_flag = df_companies.get("pine_panel_flag")
        mentions_pinus = df_companies.get("item_count_mentions_pinus")

        mask_pine = pd.Series(False, index=df_companies.index)
        if works_flag is not None:
            mask_pine = mask_pine | df_companies["works_with_pine_panel"].astype(str).str.lower().eq("yes")
        if pine_flag is not None:
            mask_pine = mask_pine | (df_companies["pine_panel_flag"].fillna(0).astype(int) == 1)
        if mentions_pinus is not None:
        ...
        total_produtos = int(df_items[df_items["nm_empresa"].isin(df_portfolio["nm_empresa"])]["product_name"].nunique())
        ...
        cols_show = [c for c in ["nm_empresa", "website_url", "status", "works_with_wood", "works_with_pine_panel", "confidence_pine_panel", "item_count_total", "item_count_wood_related", "item_count_mentions_pinus"] if c in df_view.columns]
        st.dataframe(df_view[cols_show].rename(columns={"nm_empresa":"Empresa", "website_url":"Website", "works_with_wood":"Works with wood", "works_with_pine_panel":"Works with pine panel"}), height=300)

        st.markdown("**Detalhe do portfólio por empresa**")
        empresa_sel = st.selectbox("Selecione uma empresa para ver produtos:", options=["(Nenhuma)"] + df_view["nm_empresa"].tolist())
        if empresa_sel and empresa_sel != "(Nenhuma)":
            ...
            if df_products.empty:
                ...
                cols_prod = [c for c in ["product_category", "product_name", "materials", "is_wood_related", "mentions_pinus"] if c in df_products.columns]
                st.dataframe(df_products[cols_prod].rename(columns={"product_category":"Categoria","product_name":"Produto","materials":"Materiais","is_wood_related":"Wood-related","mentions_pinus":"Menção a Pinus"}), height=300)

        st.markdown("**Mapa (placeholder)**")
        # Mantemos o mapa placeholder até termos município/coords confiáveis
        ...
        fig_p.update_layout(mapbox_style="carto-darkmatter", margin=dict(l=0, r=0, t=0, b=0), height=500)
